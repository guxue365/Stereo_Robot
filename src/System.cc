...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer, bool is_reuse_map_):mSensor(sensor), is_reuse_map(is_reuse_map_), mpViewer(static_cast<Viewer*>(NULL)),
...
//#ifdef FUNC_MAP_SAVE_LOAD
    ...
    if (!mapfilen.empty())
    {
        ...
        cerr << "find mapfile"<<endl;
    }
    else
    {
        cerr << "map file empty"<<endl;
    }
//#endif
    ...
    //Create the Map
#ifdef FUNC_MAP_SAVE_LOAD
    if (is_reuse_map==true&&!mapfile.empty() && LoadMap(mapfile))
    {
        bReuseMap = true;
    }
    else
...
#endif
void System::SaveMapSLOT(const string &filename)
{
  SaveMap(filename);
}
cv::Mat System::getFrameDrawer()
{
    unique_lock<mutex> lock(mMutexgetframe);
    return mpFrameDrawer->DrawFrame();
}
std::vector<MapPoint*> System::getmspmappoint()
{
    return mpMap->GetAllMapPoints();
}

vector<KeyFrame*> System::getsortVkf()
{
  vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
   // sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
    return vpKFs;
...
vector<MapPoint*> System::getvpKFp()
{
